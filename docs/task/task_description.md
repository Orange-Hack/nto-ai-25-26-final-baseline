...
Подробное описание структуры файлов и полей приведено в [data_description.md](data_description.md).
...
Для формирования кейса используется период в 180 дней, разделенный на два этапа:

1. **Период чистой истории** (первые 150 дней) — логи в этом окне считаются полными и наблюдаемыми.
2. **Окно инцидента** (последние 30 дней) — период, в котором произошел сбой логирования и часть событий была потеряна.
...
Примеры (3 шт.) вынесены в отдельный документ: [metric_examples.md](metric_examples.md).
